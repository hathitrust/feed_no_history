package HTFeed::Dataset::Stage::UnpackText;
...
sub run{
    ...
    my $repo_mets_path = $volume->get_repository_mets_path();
    $repo_mets_path //= "";
    unless ($repo_mets_path and -e $repo_mets_path){
        $self->set_error('MissingFile',file=>"METS",volume=>$volume->get_identifier());
        return;
    }
    ...
    if (-e $zipfile){
        ...
        $self->unzip_file_preserve_case($zipfile,$volume->get_staging_directory(),'*.txt') or return;
    }
    else{
